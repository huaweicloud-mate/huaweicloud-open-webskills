...
| 正文字号 | `14px` |
| 行高 | `22px`（14px 正文） |
| 按钮圆角 | `999px`（胶囊形），TinyButton 使用 `round` 属性 |
| 卡片圆角 | `4px`（radius-m） |

### 5. 响应式字体规范（默认值，`!important` 优先）

所有页面使用元素选择器 `h1`/`h2`/`h3`/`p` 统一控制字体：

| 断点 | h1 | h2 | h3 | p |
|---|---|---|---|---|
| 1920~1601 | 36px | 36px | 20px | 16px |
| 1600~1281 | 32px | 32px | 18px | 14px |
| 1280~1025 | 28px | 28px | 16px | 12px |
| 1024~840 | 28px | 28px | 14px | 12px |
| 839~600 | 24px | 24px | 14px | 12px |
| ≤768 | 18px | 18px | 14px | 12px |
...
只改变 `border-color: #C2C2C2`，保持文字颜色和背景不变。

## 最佳实践

### 1. 颜色使用
- **优先使用 CSS 变量**：所有颜色应通过 `var(--por-xxx)` 引用，便于主题切换和维护
- **克制使用品牌色**：`#191919` 仅用于强调和按钮
